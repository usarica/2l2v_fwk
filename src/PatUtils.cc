...
//Factorization and Rinormalization Scale
std::pair<double, double> scaleVariation(const fwlite::Event& ev){
	...
	std::vector<double> scaleVect;
        if( lheEPHandle.isValid() ){
			...
			//std::cout << "ScaleUp: " << scaleUp << "; ScaleDw: " << scaleDw << std::endl;
                 }
         }
...
double alphaVariation(const fwlite::Event& ev){
	...
	alphaVar = std::sqrt(0.75)*std::abs( local_alpha_one - local_alpha_two )*0.5; 
	return alphaVar;
}
